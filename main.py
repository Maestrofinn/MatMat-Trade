...
def moves_from_sorted_index_by_sector(
	...
	for reg in regs:
		remaining_reg_export[reg] = reference.Z.drop(columns=reg).sum(axis=1).loc[(reg, sector)]
...
def scenar_pref_europe(reloc : bool=False) -> Dict:
	...
	for sector in sectors:
		...
		remaining_reg_export_UE = remaining_reg_export["EU"]
		for j in range(nbsect):
			if totalinterfromsector[j] != 0 and remaining_reg_export_UE > 0:
				# if europe can still export some sector[i]
				if remaining_reg_export_UE > totalinterfromsector[j]:
					alloc = totalinterfromsector[j]
				else:
					alloc = reference.Z.loc[
						("EU", sector), ("FR", sectors[j])
					]
				...
				for r in regs:
					if r != "EU":
						...
						] * (1 - alloc / totalinterfromsector[j])

		for j in range(nbdemcats):
			if totalfinalfromsector[j] != 0 and remaining_reg_export_UE > 0:
				# if europe can still export some sector[i]
